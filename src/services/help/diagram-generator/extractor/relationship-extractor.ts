...
export class RelationshipExtractor {
  ...
  /**
   * Extract relationships from a type alias.
   */
  extractFromTypeAlias(
    filePath: string,
    typeName: string,
    knownTypes: Set<string>
  ): RelationshipInfo[] {
    ...
    // Get the type text and extract references
    const typeText = typeAlias.getType().getText();
    const refs = this.simplifier.extractTypeReferences(typeText);

    for (const ref of refs) {
      if (knownTypes.has(ref) && ref !== typeName && !seen.has(ref)) {
        seen.add(ref);
        relationships.push({
          from: typeName,
          to: ref,
  ...
  extractFromClass(
    ...
    if (extendedClass) {
      const typeName = this.extractTypeName(extendedClass.getText());
      if (typeName && knownTypes.has(typeName) && !seen.has(typeName)) {
        seen.add(typeName);
        relationships.push({
          from: className,
    ...
    for (const impl of classDecl.getImplements()) {
      const typeName = this.extractTypeName(impl.getText());
      if (typeName && knownTypes.has(typeName) && !seen.has(typeName)) {
        seen.add(typeName);
        relationships.push({
          from: className,
    ...
    for (const prop of classDecl.getProperties()) {
      ...
      const refs = this.simplifier.extractTypeReferences(typeText);
      for (const ref of refs) {
        if (knownTypes.has(ref) && ref !== className && !seen.has(ref)) {
          seen.add(ref);
          relationships.push({
            from: className,
            to: ref,
            type: 'uses',
            label: `has ${prop.getName()}`,
          });
        }
      }
    }
