...
export class ClassDiagramBuilder {
  ...
  /**
   * Create a new builder with the same config.
   */
  clone(): ClassDiagramBuilder {
    const cloned = new ClassDiagramBuilder(this.config, this.methodSelector);
    for (const [name, info] of this.classes) {
      cloned.classes.set(name, { ...info });
    }
    cloned.relationships = [...this.relationships];
    cloned.sources = new Set(this.sources);
    return cloned;
  }
}
