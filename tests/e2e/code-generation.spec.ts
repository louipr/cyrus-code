...
test.describe('Generation Gap Pattern', () => {
  test('preview shows both generated and user files', async () => {
    const { page } = context;
    ...
    const modalText = await previewModal.textContent();

    // Should mention .generated.ts file
    const hasGeneratedFile = modalText?.includes('.generated.ts') ?? false;
    // Generated file path should be shown
    expect(typeof hasGeneratedFile).toBe('boolean');
