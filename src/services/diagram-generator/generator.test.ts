...
import { DefaultMethodSelector } from '../../domain/diagram/method-selector.js';
...
describe('C4DiagramGenerator', () => {
  ...
  describe('generateForInterface', () => {
    ...
    it('should include related types when specified', () => {
      // Note: ComponentSymbol and Connection are now in domain/symbol/schema.ts
      ...
      assert.ok(result.diagram.classes.length >= 2, 'Should have at least 2 classes');
      assert.ok(result.rendered.includes('ComponentQuery') || result.rendered.includes('ResolveOptions'),
